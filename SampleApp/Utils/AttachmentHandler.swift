...
class AttachmentHandler: NSObject {
    ...
    func showAttachmentActionSheet(vc: UIViewController) {
        ...
        if let popoverController = actionSheet.popoverPresentationController {
            popoverController.sourceView = vc.view // The view containing the anchor rectangle for the popover.
            popoverController.sourceRect = CGRect(x: vc.view.bounds.midX, y: vc.view.bounds.midY, width: 0, height: 0)
            popoverController.permittedArrowDirections = [] // No arrow directions; present the popover centered.
        }
    ...
    func openCamera() {
        if UIImagePickerController.isSourceTypeAvailable(.camera) {
            ...
            // Check if the device is an iPad
            if let popoverController = myPickerController.popoverPresentationController {
                popoverController.sourceView = currentVC?.view // The view containing the anchor rectangle for the popover.
    ...
    func addAlertForSettings(_ attachmentTypeEnum: AttachmentType) {
        ...
        // Check if the device is an iPad
        if let popoverController = cameraUnavailableAlertController.popoverPresentationController {
            popoverController.sourceView = currentVC?.view // The view containing the anchor rectangle for the popover.
            popoverController.sourceRect = (currentVC?.view.bounds)!
            popoverController.permittedArrowDirections = [] // No arrow directions; present the popover centered.
        }
